...
const PreamblePostambleRecordPage: React.FC = () => {
  ...
  const decodeRecordedAudio = () => {
    ...
    // Pass entire recording with preamble/postamble for full decode
    const allSamples = recordedSamplesRef.current

    console.log(`Decoding: passing ${allSamples.length} samples (full recording with preamble/postamble)`)

    // Send to decoder worker for async decoding with full sync detection
