use transmitwave_core::sync::{
    ...
    generate_preamble, generate_postamble_signal, barker_code, DetectionThreshold,
};
...
fn test_detect_preamble_with_chirp() {
    ...
    let detected = detect_preamble(&samples, DetectionThreshold::Adaptive);
    assert!(
        detected.is_some(),
...
fn test_detect_postamble_with_chirp() {
    ...
    let detected = detect_postamble(&samples, DetectionThreshold::Adaptive);
    assert!(
        detected.is_some(),
...
fn test_full_frame_detection() {
    ...
    // Detect preamble
    let preamble_pos = detect_preamble(&samples, DetectionThreshold::Adaptive);
    ...
    // Detect postamble
    let postamble_pos = detect_postamble(&samples, DetectionThreshold::Adaptive);
...
fn test_fft_correlation_index_mapping_with_preamble() {
    ...
    let detected = detect_preamble(&signal, DetectionThreshold::Adaptive);
