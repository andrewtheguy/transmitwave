...
use testaudio_core::{Decoder, Encoder, DecoderSpread, EncoderSpread, DecoderCss, EncoderCss, resample_audio, stereo_to_mono, SAMPLE_RATE};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(command) = cli.command {
        match command {
            Commands::Encode { input, output, no_spread, css } => {
                if css {
            ...
            Commands::Decode { input, output, no_spread, css } => {
                if css {
...
fn encode_css_command(input_path: &PathBuf, output_path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    // Read input binary file
    let data = std::fs::read(input_path)?;
    println!("Read {} bytes from {}", data.len(), input_path.display());
    ...
    let samples = encoder.encode(&data)?;
    println!("Encoded with CSS to {} audio samples", samples.len());
...
fn decode_css_command(input_path: &PathBuf, output_path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    // Read WAV file
    let file = File::open(input_path)?;
    let mut reader = hound::WavReader::new(file)?;
    ...
    let data = decoder.decode(&samples)?;
    println!("Decoded {} bytes (CSS)", data.len());
