...
fn test_encode_spread_default_chip_duration() {
    ...
    let output_text =
        run_transmitwave(&["encode", input.to_str().unwrap(), output.to_str().unwrap()]);

    // Should successfully encode with multi-tone FSK
    assert!(
        output_text.contains("multi-tone FSK") || output_text.contains("Encoded"),
...
fn test_decode_spread_default_chip_duration() {
    ...
    // Encode
    run_transmitwave(&["encode", input.to_str().unwrap(), encoded.to_str().unwrap()]);
    ...
    assert!(
        output_text.contains("Decoded") || output_text.contains("bytes"),
        "Decode should succeed but got: {}",
        output_text
    );
...
fn test_positional_args_encode_decode() {
    ...
    let encode_output = run_transmitwave(&[input.to_str().unwrap(), encoded.to_str().unwrap()]);

    assert!(
        encode_output.contains("Encoded"),
        "Positional encode should succeed. Got: {}",
        encode_output
    );

    // Decode using positional args (auto-detects .wav as input)
    let decode_output = run_transmitwave(&[encoded.to_str().unwrap(), decoded.to_str().unwrap()]);

    assert!(
        decode_output.contains("Decoded"),
        "Positional decode should succeed. Got: {}",
        decode_output
    );
}
...
fn test_roundtrip_consistency() {
    ...
    // Verify roundtrip
    let decoded_content = fs::read_to_string(&decoded).expect("Failed to read decoded output");

    assert_eq!(
        decoded_content, input_text,
        "Roundtrip failed: expected '{}' but got '{}'",
...
fn test_file_extension_auto_detection() {
    ...
    let output_text = run_transmitwave(&[input.to_str().unwrap(), output.to_str().unwrap()]);

    // Should successfully encode
    assert!(
        output_text.contains("chip_duration") || output_text.contains("Encoded"),
        "Auto-detection failed for .bin file. Got: {}",
        output_text
    );
...
fn test_different_input_sizes_encode_successfully() {
    ...
    assert!(
        small_output.contains("Encoded") || small_output.contains("audio samples"),
        "Small input should encode successfully. Got: {}",
        small_output
    );
    assert!(
        medium_output.contains("Encoded") || medium_output.contains("audio samples"),
        "Medium input should encode successfully. Got: {}",
        medium_output
    );
    ...
    assert!(
        size_small > 5_000,
        "Small output too small: {} bytes",
        size_small
    );
    assert!(
        size_medium > 5_000,
        "Medium output too small: {} bytes",
        size_medium
    );
    assert!(
        size_small < 500_000,
        "Small output too large: {} bytes",
        size_small
    );
    assert!(
        size_medium < 500_000,
        "Medium output too large: {} bytes",
        size_medium
    );
}
